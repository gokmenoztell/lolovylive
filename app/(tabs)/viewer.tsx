...
export default function Views() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setVisibleComments((prevVisibleComments) => {
        ...
        if (
          prevVisibleComments.some((comment) => comment.id === newComment.id)
        ) {
  ...
  useFocusEffect(
    useCallback(() => {
      setCameraActive(false); // Önce kamerayı kapat
      setTimeout(() => setCameraActive(true), 100); // 100ms sonra yeniden aç
      return () => setCameraActive(false); // Sayfa değişince kapat
    }, [])
  );
